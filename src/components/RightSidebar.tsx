...
import { useAppStore } from '@/lib/store/useAppStore'; // Import zustand store
...
export default function RightSidebar() {
  ...
  // --- Remove Local State for Messages, Loading, Error ---
  ...
  // OLD handleSend logic - remove this
  // const handleSend = async () => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100 dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700">
      ...
      <div ref={chatContainerRef} className="flex-grow bg-white dark:bg-gray-800 mb-2 overflow-y-auto p-3 space-y-3">
          ...
          <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              ...
              {msg.parts?.[0]?.text ? (
                 ...
                 // Handle cases where parts might be missing temporarily during streaming or error
        ...
        {/* Use chatError from store - maybe display centrally or just log? Displaying as a message for now */} 
        {/* Error messages are now added directly to chatMessages by the store action 
         {chatError && (
           <div className="flex justify-start">
              <div className="max-w-[80%] p-2 px-3 rounded-lg text-sm bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 shadow-sm">
                 {chatError}
             </div>
           </div>
         )} 
